...
- [ ] 1. Update environment and dependency files
  ...
  - [ ] 1.2 Update `analyzer/requirements.txt`
  ...
  - [ ] 1.3 Update `faulty-workload/requirements.txt`
...
- [ ] 2. Create `analyzer/datadog_mcp.py`
  - [ ] 2.1 Define `MCPEvidence` dataclass and custom exceptions
  ...
  - [ ] 2.2 Implement `_get_mcp_client()` with stdio and HTTP transport
  ...
  - [ ] 2.3 Implement `_normalize_logs()` and `_normalize_traces()` helpers
...
- [ ] 3. Update `analyzer/bundle.py`
  - [ ] 3.1 Change `build_bundle` signature to accept `MCPEvidence`
...
- [ ] 4. Update `analyzer/bedrock.py`
  - [ ] 4.1 Update `_REQUIRED_KEYS` and `build_prompt()`
  ...
  - [ ] 4.2 Update `_init_llmobs()` to use agent mode
...
- [ ] 5. Update `analyzer/app.py`
  - [ ] 5.1 Swap `cloudwatch` import for `datadog_mcp` and update `AnalyzeResponse`
  ...
  - [ ] 5.2 Update `rca_analyze()` orchestration
...
- [ ] 6. Checkpoint — local unit and property tests
...
- [ ] 7. Update `docker-compose.yml`
  ...
  - [ ] 7.3 Update `faulty-workload` service
...
- [ ] 8. Update `scripts/smoke_test.sh`
  - [ ] 8.1 Update expected response fields and assertions
...
- [ ] 9. Checkpoint — run local smoke test end-to-end
...
- [ ] 10. Implement remaining Hypothesis property tests
  - [ ] 10.1 Write property test — Property 1: Structured log fields completeness
  ...
  - [ ] 10.2 Write property test — Property 2: Trace ID context propagation
  ...
  - [ ] 10.3 Write property test — Property 3: Triage result valid enum
  ...
  - [ ] 10.4 Collect all property tests into `analyzer/test_properties.py`
...
- [ ] 11. Checkpoint — run all property tests
...
- [ ] 12. Create Kubernetes manifests for Datadog
  - [ ] 12.1 Create `k8s/datadog-agent.yaml`
  ...
  - [ ] 12.2 Create `k8s/datadog-mcp.yaml`
...
- [ ] 13. Update existing Kubernetes manifests
  - [ ] 13.1 Update `k8s/analyzer.yaml`
    - Remove `LOG_GROUP`, `METRIC_NAMESPACE` env vars
  ...
  - [ ] 13.2 Update `k8s/faulty-workload.yaml`
...
- [ ] 14. Remove obsolete CloudWatch assets
  - [ ] 14.1 Delete `k8s/otel-collector.yaml`
    - This DaemonSet is replaced by the Datadog Agent DaemonSet
    - _Requirements: 2.5_

  - [ ] 14.2 Delete `cloudwatch/` folder (`alarms.json`, `dashboard.json`)
    - _Requirements: 2.5_
...
- [ ] 15. Delete `analyzer/cloudwatch.py`
...
- [ ] 16. Update `analyzer/templates/index.html`
  - [ ] 16.1 Update `renderRCA()` JavaScript function
