...
- [ ] 1. Create fault profile registry and wire into app startup
  - [ ] 1.1 Create `faulty-workload/profiles.py` with fault profile registry
  ...
  - [ ] 1.2 Modify `faulty-workload/app.py` to apply profile at startup
  ...
  - [ ]* 1.3 Write property test for profile resolution (Property 1)
  ...
  - [ ]* 1.4 Write property test for env-var override precedence (Property 5)
...
- [ ] 2. Extend logger with `workload_family` field
  - [ ] 2.1 Modify `faulty-workload/logger.py` to include `workload_family` in JSON output
  ...
  - [ ]* 2.2 Write property test for log format completeness (Property 2)
  ...
  - [ ]* 2.3 Write property test for trace ID propagation (Property 3)
  ...
  - [ ]* 2.4 Write property test for error-type stability (Property 4)
...
- [ ] 3. Extend metrics with `workload_family` label
  - [ ] 3.1 Modify `faulty-workload/metrics.py` to add `workload_family` label to all counters and histogram
  ...
  - [ ] 3.2 Update `faulty-workload/app.py` metric call sites to use `.labels(workload_family=...)`
  ...
  - [ ]* 3.3 Write unit tests for metrics `workload_family` label
...
- [ ] 5. Create K8s deployment manifests for variant workloads
  - [ ] 5.1 Create `k8s/observable-workloads.yaml` with Deployment and Service for each variant
...
- [ ] 6. Add docker-compose services for local development
  - [ ] 6.1 Add `customers-workload`, `orders-workload`, `auth-workload` services to `docker-compose.yml`
  ...
  - [ ] 6.2 Update Datadog agent `DD_PROMETHEUS_SCRAPE_CHECKS` to include variant endpoints
    - Add scrape URLs for all three variant services
    - _Requirements: 4.1, 5.2_

- [ ] 7. Integration wiring and final validation
  - [ ]* 7.1 Write unit tests for profile application and fallback behavior
  ...
  - [ ]* 7.2 Write integration test verifying variant telemetry is distinguishable
